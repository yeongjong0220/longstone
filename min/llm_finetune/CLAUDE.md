...
- 입력 분포 정의: `min/min_dev_park/live_ai_coach_v3_api.py:271-275` (`request_feedback`의 llm_prompt)
- 학습 데이터 형식: [data/schema.md](data/schema.md) (Alpaca instruction/input/output, 다중 자세 일반화)
- 본 학습용 `data/sft.jsonl`은 미생성 — 합성 단계 별도 plan 예정
...
## 후속 작업 (별도 plan)
- 합성 SFT 데이터셋 구축 (시드 작성 → 외부 LLM augment → 검수)
- 본 학습 (HCX-full 또는 Gemma-LoRA 두 갈래)
- `min/min_dev_park/live_ai_coach_v2.py`에 PeftModel 통합
