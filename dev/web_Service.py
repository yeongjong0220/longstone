...
# [환경 설정] 윈도우 환경 메모리 및 라이브러리 충돌 방지
...
print("[진행] 로컬 메모리에 Gemma 모델을 적재하는 중입니다...")
...
while cap.isOpened():
    ...
    if key == ord('f'):
        ...
        if error_frames < 5:
            print("[알림] 충분한 오류 데이터가 모이지 않았습니다. 자세 이탈 후 다시 시도해주세요.")
            continue
            
        print("\n[진행] AI 강사가 방금 수행한 동작을 분석하고 있습니다... (잠시만 기다려주세요)\n")
