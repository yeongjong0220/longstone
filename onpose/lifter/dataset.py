...
def _rotation_align_3d(src: np.ndarray, dst: np.ndarray) -> np.ndarray:
    """3D Rodrigues 회전 — src 벡터를 dst 벡터에 정렬."""
    src = src / max(float(np.linalg.norm(src)), 1e-8)
    dst = dst / max(float(np.linalg.norm(dst)), 1e-8)
    v = np.cross(src, dst)
    ...
    R = np.eye(3, dtype=np.float32) + K + K @ K * ((1 - c) / (s * s))
    return R.astype(np.float32)


def smooth_keypoints(coords: np.ndarray, window: int = 7, polyorder: int = 2) -> np.ndarray:
    """시간축 Savitzky-Golay 스무딩 (T × J × D 시퀀스).

    AI Hub GT 의 시간축 jitter 완화. window=7, polyorder=2.
    시퀀스가 window 보다 짧으면 자동 축소, polyorder+2 미만이면 통과.
    """
    if coords.shape[0] < 5:
        return coords
    try:
    ...
    if w < polyorder + 2:
        return coords
    out = savgol_filter(coords, window_length=w, polyorder=polyorder, axis=0, mode="nearest")
...
def normalize_skeleton(
    ...
    neck_joint: str = "Neck",
    align_torso: bool = True,
) -> np.ndarray:
    """Neck-Hip 축 정규화.

    단계:
      1. Hip 을 origin 으로 평행이동
      2. torso 길이 (Neck - Hip 의 시퀀스 중앙값) 으로 스케일
      3. (옵션) Neck-Hip 축을 +y 로 회전 정렬

    Args:
        coords: (T, J, D) — T 프레임, J=15 관절, D=2 또는 3
        align_torso: True 면 3단계까지 적용. False 면 1, 2 단계만.
    """
    ...
    # 1) 평행이동 — Hip 을 origin 으로
    root = arr[:, idx[root_joint]: idx[root_joint] + 1, :]
    arr = arr - root

    # 2) 스케일 — torso 길이 (Neck - Hip) 시퀀스 중앙값 으로 통일
    torso_len = np.linalg.norm(
        arr[:, idx[neck_joint], :] - arr[:, idx[root_joint], :], axis=-1
    )
    ...
    # 3) 회전 — Neck-Hip 축을 +y 로 정렬 (시퀀스 평균 방향 기준; 프레임별 회전은 떨림)
    if align_torso and D >= 2:
        torso_vec = arr[:, idx[neck_joint], :] - arr[:, idx[root_joint], :]
        mean_dir = np.nanmean(torso_vec, axis=0)
        norm = float(np.linalg.norm(mean_dir))
        if np.isfinite(norm) and norm > 1e-6:
            mean_dir = (mean_dir / norm).astype(np.float32)
            if D == 2:
                target = np.array([0.0, 1.0], dtype=np.float32)
                R = _rotation_align_2d(mean_dir, target)
            else:  # D == 3
                target = np.array([0.0, 1.0, 0.0], dtype=np.float32)
                R = _rotation_align_3d(mean_dir, target)
            arr = arr @ R.T   # (T, J, D) @ (D, D) — last dim 에 회전 적용
...
class PilatesTemporalLifterDataset(Dataset):
    ...
    def _load_clip(self, idx: int) -> Dict[str, Any]:
        ...
        # GT (3D) 시간축 스무딩 — AI Hub keypoint 의 jitter 완화 (사용자 피드백 반영)
        y3d = smooth_keypoints(y3d, window=7, polyorder=2)
        # 입력 2D 는 약한 스무딩만 (학습 안정화, 추론 noise robustness 보존)
        x2d = smooth_keypoints(x2d, window=5, polyorder=2)

        # Neck-Hip 축 정규화 (평행이동 + torso 스케일 + 축 회전 정렬)
        x2d = normalize_skeleton(x2d, align_torso=True)
        y3d = normalize_skeleton(y3d, align_torso=True)
