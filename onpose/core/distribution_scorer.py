...
from .pose_targets import _angle_subscore
...
def score_frame_distribution(angles: Dict[str, float], rubric: DistributionRubric) -> Dict:
    ...
    for k, spec in rubric.angles.items():
        ...
        if curve is not None:
            # 자세별 채점 곡선 (peak / at_most / at_least) — 단일 출처
            sub, info = _angle_subscore(v, curve)
            target = info["ideal"]; diff = info["diff"]
            ok = info["ok"]; direction = info["direction"]; z = 0.0
        else:
            # 레거시 z-score 가우시안 경로 (curve 없는 각도용)
            mean = spec.target_deg
            std = rubric.angle_std.get(k, 5.0)
            z = (v - mean) / std
            sub = _gaussian_score(z)
            target = mean; diff = abs(v - mean)
            ok = diff <= spec.tolerance_deg
            direction = None if ok else ("더 펴 주세요" if v < mean else "더 굽혀 주세요")
        details[k] = {
            "value": round(v, 2),
            "target": target,
            "diff": round(diff, 2),
...
def score_sequence_distribution(angle_frames: Sequence[Dict[str, float]],
    ...
    per_angle_mean_pen: Dict[str, float] = {}   # curve 기반 평균 벌점거리(정답까지 더 조절해야 할 °)
    for k in rubric.angles.keys():
        ...
        pens = [f["details"][k]["diff"] for f in per_frame]
        ...
        per_angle_mean_pen[k] = round(float(np.mean(pens)), 2)
    ...
    return {
        ...
        "per_angle_mean_pen": per_angle_mean_pen,
